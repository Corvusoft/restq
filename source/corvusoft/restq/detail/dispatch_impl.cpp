...
namespace restq
{
    namespace detail
    {
        ...
        const Bytes DispatchImpl::SUSPENDED = String::to_bytes( "suspended" );
        ...
        shared_ptr< Logger > DispatchImpl::m_logger = nullptr;
        
        shared_ptr< Service > DispatchImpl::m_service = nullptr;
        
        shared_ptr< Repository > DispatchImpl::m_repository = nullptr;
        
        void DispatchImpl::route( const Bytes status )
        {
            auto query = make_shared< Query >( );
            query->set_limit( 1 );
            query->set_exclusive_filter( "type", STATE );
            query->set_exclusive_filter( "status", status );
            
            //if ( status == SUSPENDED )
            //{
            //    without this ability we keep reading the same state over and over.
            //    query->set_ordering( "redeliver-at", Query::ASCENDING default);
            //}
            
            m_repository->read( query, DispatchImpl::router );
        }
        ...
        void DispatchImpl::router( const shared_ptr< Query > query )
        {
            if ( query->has_failed( ) )
            {
            ...
            auto state_status = states.back( ).lower_bound( "status" )->second;
            
            if ( state_status == SUSPENDED )
            {
                const auto timestamp = system_clock::from_time_t( stoul( String::to_string( states.back( ).lower_bound( "redeliver-at" )->second ) ) );
                
                if ( timestamp > system_clock::now( ) )
                {
                    return;
                }
            }
            ...
            m_repository->update( { { "status", INFLIGHT } }, query, [ state_key ]( const shared_ptr< Query > query )
            {
                if ( query->has_failed( ) )
                {
                    //log if it was a conflict as-well.
                ...
                m_repository->read( query, [ state_key, states ]( const shared_ptr< Query > query )
                {
                    ...
                    else
                    {
                        ...
                        {
                            const auto redelivery_interval = seconds( stoul( String::to_string( states.back( ).lower_bound( "redelivery-interval" )->second ) ) );
                            const auto delay = system_clock::now( ) + redelivery_interval;
                            
                            status = SUSPENDED;
                            change.insert( make_pair( "redeliver-at", String::to_bytes( ::to_string( system_clock::to_time_t( delay ) ) ) ) );
                            change.insert( make_pair( "max-delivery-attempts", String::to_bytes( ::to_string( --max_delivery_attempts ) ) ) );
                            
                            log( Logger::WARNING, String::format( "Failed to dispatch message '%s' to subscription '%s', attempting redelivery after %lu seconds.", message_key.data( ), subscription_key.data( ), redelivery_interval.count( ) ) );
                        }
                        else
                        {
