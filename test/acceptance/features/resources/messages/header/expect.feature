...
Feature: Expect request-header field
    ...
    Scenario Outline: Valid expect request-header field.
        ...
        When I perform a HTTP "POST" request to "/messages" with headers "Content-Type: text/plain, Accept: application/json, Expect: 100-continue, Host: localhost:1984":
        """
    ...
    Scenario Outline: Invalid expect request-header field.
        Given I have started a message exchange
        When I perform a HTTP "POST" request to "/messages" with headers "Content-Type: text/plain, Accept: application/json, Expect: %^&*&!@(]^%$, Host: localhost:1984":
        """
        " ftp://localhost/reading "
        """
        Then I should see a response status code of "417" "Expectation Failed"
